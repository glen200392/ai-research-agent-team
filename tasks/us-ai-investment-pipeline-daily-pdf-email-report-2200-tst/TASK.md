...
- description: 'Stage 3 — 基本面摘要 (Fundamental Snapshot): 對篩選標的執行快速基本面分析：P/E、Forward
  ...
  format_guide: '輸出 JSON：{stocks: [{ticker, fundamental_score, valuation: {pe, forward_pe,
    ...
    key_strengths, key_risks, recommendation: BUY|HOLD|AVOID,
    en_global_perspective: {wall_street_consensus: string（Buy/Hold/Sell ratio from Bloomberg consensus），
    consensus_pt_usd: string（average price target from sell-side analysts），
    earnings_revision_trend: string（EPS estimate revision direction: upward/downward/stable），
    competitive_moat_en: string（key moat vs global AI competitors in English, e.g. CUDA lock-in / hyperscaler dependency）}}]}。繁體中文基本面摘要。'
